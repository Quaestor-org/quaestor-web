...
export function QuizForm({ lessonId, questions }: { lessonId: string; questions: ClientQuestion[] }) {
  ...
  const form = useForm({
    defaultValues: {
      answers: questions.reduce((acc, q) => ({ ...acc, [q.id]: '' }), {} as Record<string, string>),
    },
  ...
  return (
    ...
    >
        ...
        <Card key={q.id} className="border-zinc-200 shadow-sm">
          <CardContent className="pt-6">
            ...
            <form.Field
              name={`answers.${q.id}`}
              validators={{
                onChange: ({ value }) => (!value ? 'Please select an answer' : undefined),
              }}
              children={(field) => (
                  ...
                  >
                    {q.answers.map((a) => (
                      <div key={a.id} className="flex items-center space-x-3">
                        ...
                        <Label htmlFor={a.id} className="font-normal text-base text-zinc-700 cursor-pointer">
                          {a.text}
                        </Label>
                      </div>
                    ))}
      ...
      {mutation.isError && (
        <p className="text-sm text-red-500 mt-2">Error submitting quiz. Please try again.</p>
      )}
    </form>
  );
}
